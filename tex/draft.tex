...
\begin{equation}\label{eq:Poisson}
  \begin{aligned}
    -\nabla\cdot( \varepsilon_0(1+\chi) \nabla \phi) & = q(\phi,p) \; \text{in} \Omega \\
  \end{aligned}
\end{equation}

We set two variants of boundary conditions:
\begin{itemize}
  \item symmetric cell with ideally polarizable electrode and fixed charges:
        \begin{equation}
          \label{eq:symmcell}
          \begin{aligned}
            \varepsilon_0(1+\chi)\nabla\phi \cdot \vec n |_{x=0} & = q_s  \\
            \varepsilon_0(1+\chi)\nabla\phi \cdot \vec n |_{x=L} & = -q_s
          \end{aligned}
        \end{equation}
...
Let $\Lambda(x) = \ln \frac{\sinh(x)}{x}$ be the antiderivative of the
Langevin function $\mathcal L (x)=\coth(x)-\frac1x$.

We set the polarization free energy contribution as
$$
  \rho\psi^{Pol} = -k_BT\sum_{\alpha} n_\alpha\Lambda(\delta_\alpha |E|)
$$
where $E=\nabla \phi$ is the electric field.

Let $\mathcal W(x)= 3\frac{\mathcal L(x)}{x}$.

Then, for
$$
  \chi = \sum_\alpha v_\alpha n_\alpha \chi_\alpha^0\mathcal W\left(\frac{\delta_\alpha |E|}{k_BT}\right)
$$
and
$$
...
$$
  P(E)= -\frac{\partial \rho\psi^{pol}}{\partial E}= \varepsilon_0 \chi E
$$

Then we have for the mole fractions
\begin{align}\label{eq:y}
  y_\alpha(\phi,p)= y_\alpha^E\exp\left(
  - \frac{z_\alpha e (\phi-\phi^E)
      + v_\alpha(p-p^E)}{k_BT}
  + \Lambda\left(\frac{\delta_\alpha |E|}{k_BT}\right)
  \right).
\end{align}
We can calculate the space charge density as
\begin{align}\label{eq:rho}
  q(\phi,p) = e\sum_{\alpha=1}^n z_\alpha n_\alpha = e\sum_{\alpha=1}^N z_\alpha \frac{y_\alpha}{\sum_{\alpha=0}^N v_\alpha y_\alpha}.
\end{align}
\medskip
...
From the space charge density $q(\phi, p)$ for given solution with applied potential $U$,
one calculates the double layer charge
\begin{align}
  \label{eq:spacecharge}
  Q_{dl}=Q_{dl}(U)=\int_\Omega q\; d\omega.
\end{align}
